...
namespace Components
{
    public partial class GridManager
    {
        ...
        private Tile DrawTile(Rect rect, Tile tile)
        {
            ...
            if (preview == null)
            {
                Debug.LogWarning("AssetPreview.GetAssetPreview returned null for tile: " + tile.name);
            }
            else
            {
                rect = rect.Padding(3);
        ...
        private void CreateGrid(int sizeX, int sizeY)
        {
            ...
            for (int y = 0; y < _gridSizeY; y++)
            {
                ...
                GameObject tileNew =
                    PrefabUtility.InstantiatePrefab(tilePrefabRandom,
                        transform) as GameObject; //Instantiate rand prefab
